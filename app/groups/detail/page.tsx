...
import {
  Search, Settings, Lock, Trophy, Share2, ChevronLeft,
  ShieldCheck, Bell, Trash2, X, Eye,
...
function GroupDetailPage() {
  ...
  // [추가] 비공개 그룹 초대 코드 처리 상태
  const [inputInviteCode, setInputInviteCode] = useState("");
  const [isJoiningWithCode, setIsJoiningWithCode] = useState(false);

  // URL에 ?code=... 파라미터가 파싱되어 들어온 경우 자동 입력
  useEffect(() => {
    const codeFromUrl = searchParams.get("code");
    if (codeFromUrl) {
      setInputInviteCode(codeFromUrl);
    }
  }, [searchParams]);

  // [API] 초대 코드로 비공개 그룹 즉시 가입
  const handleJoinWithCode = async () => {
    ...
    try {
      const res = await fetch(`https://diveon.net/api/groups/${groupId}/members`, {
        ...
        body: JSON.stringify({ inviteCode: inputInviteCode.trim() }) // API 명세 초대코드 키
      });

      if (res.ok) {
        alert("비공개 그룹 검증이 완료되어 그룹에 가입되었습니다!");
        fetchGroupData(); // 그룹 정보 재로드 -> 멤버 상태(myStatus="member")로 변경됨
        fetchMembers();
      } else {
        const err = await res.json();
        alert(`가입 실패: ${err.message || err.detail || "유효하지 않거나 만료된 초대 코드입니다."}`);
      }
    } catch (error) {
      console.error("초대 코드 가입 통신 오류:", error);
    ...
    <div className="min-h-screen bg-white text-slate-900 font-sans">
      ...
      <main className="container mx-auto max-w-[1600px] pt-6 grid grid-cols-1 md:grid-cols-12 gap-6 px-4 pb-12">
        ...
        <section className="col-span-12 md:col-span-8 space-y-6">
          ...
          {/* 🔒 [핵심] 비공개 그룹 접근 차단 락 스크린 (그룹장/멤버가 아닌 경우) */}
          {isPrivate && !isGroupLeader && myStatus !== "member" ? (
            <Card className="border-amber-200/80 bg-gradient-to-b from-amber-50/40 via-white to-white shadow-lg rounded-2xl overflow-hidden p-8 text-center space-y-6 animate-in fade-in zoom-in-95 duration-300">
              <div className="w-20 h-20 bg-amber-100/80 rounded-full flex items-center justify-center mx-auto text-amber-600 shadow-inner">
                <Lock className="w-10 h-10" />
              </div>

              <div className="space-y-2 max-w-md mx-auto">
                <Badge variant="outline" className="bg-amber-100 text-amber-800 border-amber-300 font-bold px-3 py-0.5 text-xs">
                  비공개 탐사 기지
                </Badge>
                <h2 className="text-2xl font-black text-slate-900 tracking-tight">
                  {groupData?.title || "비공개 그룹"}
                </h2>
                <p className="text-sm text-slate-500 leading-relaxed">
                  이 그룹은 초대를 받은 대원만 입장할 수 있는 보안 구역입니다.<br />
                  전달받으신 <span className="font-bold text-slate-800">초대 코드</span>를 입력하여 접근 권한을 인증하세요.
                </p>
              </div>
              ...
              <div className="max-w-md mx-auto flex gap-2 pt-2">
                <Input
                  type="text"
                  placeholder="초대 코드 입력 (예: INV-8F3K9A)"
                  value={inputInviteCode}
                  onChange={(e) => setInputInviteCode(e.target.value)}
                  className="h-11 bg-white border-slate-200 font-mono text-sm uppercase text-center tracking-wider focus-visible:ring-indigo-500 rounded-xl shadow-sm"
                  onKeyDown={(e) => e.key === "Enter" && handleJoinWithCode()}
                />
                <Button
                  onClick={handleJoinWithCode}
                  disabled={isJoiningWithCode || !inputInviteCode.trim()}
                  className="h-11 px-6 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-md shrink-0 transition-all"
                >
                  {isJoiningWithCode ? "인증 중..." : "입장하기"}
                </Button>
              </div>

              <div className="pt-4 border-t border-slate-100 max-w-md mx-auto flex justify-between items-center text-xs text-slate-400 font-medium">
                <span>그룹 리더: <strong className="text-slate-600">{groupData?.leader || "알 수 없음"}</strong></span>
                <span>멤버 수: <strong className="text-slate-600">{groupData?.stats?.memberCount || 0}명</strong></span>
              </div>
            </Card>
          ) : (
